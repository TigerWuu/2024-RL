...
class GridWorld:
    ...
    def reset(self) -> int:
        ...
        if self.__init_pos_list:
            self.__current_state = self.__init_pos_list[np.random.randint(len(self.__init_pos_list))] # randomly select a state from init_pos_list
        else:
            self.__current_state = np.random.randint(len(self.__state_list)) # if no init_pos_list, randomly select a state
        return self.__current_state

...
class GridWorld:
    ...
    def __init__(
        ...
        if len(self._portal_state) == 2:
            self.portal_next_state[self._state_list[self._portal_state[0]]] = self._state_list[self._portal_state[1]]
            self.portal_next_state[self._state_list[self._portal_state[1]]] = self._state_list[self._portal_state[0]]
    ...
    def step(self, action: int) -> tuple:
        ...
        state_coord = self._state_list[state]
        if self._step_count > self.max_step:
            truncated = True # should I reset the environment? reset is called from outside?
        else:
            truncated = False
        ...
        next_state = self._state_list.index(next_state_coord) # index of the next state
        # check if the next state is the lava state
        if self._is_lava_state(next_state_coord):
            next_init_state = self.reset()
            return next_init_state, self.step_reward, True, False
        
        # non-terminal state
        # check if the next state is the bait state
        if self._is_bait_state(next_state_coord):
            self.bite()
            self._current_state = next_state
            return next_state, self._bait_reward, False, truncated
        # check if the next state is the key state
        if self._is_opened:
            self._current_state = next_state 
            return next_state+len(self._state_list), self.step_reward, False, truncated
        ...
        self._current_state = next_state 
        return next_state, self.step_reward, False, truncated # what the fuck is truncated?
    ...
    def reset(self) -> int:
        ...
        # place bait if the bait is bitten
        if self._is_baited:
            self.place_bait()
        # close door if the door is opened
        if self._is_opened:
            self.close_door()
        # choose the initial state, the state should be the left of the lava and door state, and the goal state would be the right of the lava and door state
        # initial state should not be initialized in the door, key, bait, and lava state
        # TA has done everything for us.....
        self._current_state = self._init_states[np.random.randint(len(self._init_states))] # randomly select a state from init_states
        return self._current_state    
    #############################

...
class My2048Env(gym.Env):
    ...
    def __init__(self):
        ...
        self.set_illegal_move_reward(-10) # if illegal reward too small, 
    ...
    def step(self, action):
        ...
        try:
            ...
            # empty_tile craete positive reward
            empty_tile = 0
            ...
            if reward > 0:
                empty_weight = np.log2(self.highest())/10 # the higher the score, the more important the empty tile
                pre_weight = 1/np.log2(self.highest()) # the higher the score, the less important the shape of board
            else:
                empty_weight = 0
                pre_weight = 0


            # empty_weight = np.log2(self.highest())/10 # no matter how , the agent can get reward -> doesn't make sense
            weighted_value = pre_weight * weight * self.Matrix
            extra_reward = np.linalg.norm(weighted_value)
            reward += (extra_reward+empty_tile*empty_weight)
            ...
            self.foul_count += 1
            if self.foul_count >= self.max_foul_count:
                done = True
